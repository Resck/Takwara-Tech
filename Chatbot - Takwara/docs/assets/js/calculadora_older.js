// docs/assets/js/calculadora.js - VERSÃO COM EXPORTAÇÃO CSV E IMAGENS DINÂMICAS

document.addEventListener('DOMContentLoaded', () => {
    // A base de dados para os menus suspensos
    const DOME_DATA = {
        "Icosahedron": {
            "V1": {"truncation": ["2/3"]},
            "V2": {"truncation": ["1/2"]},
    ...
    // --- NOVO --- Mapeamento de frequências para os seus ficheiros de imagem
    // O código irá procurar as imagens nesta pasta.
    const DOME_IMAGE_PATH = 'assets/images/domes/';
    const DOME_IMAGES = {
        // Frequências do Icosaedro
        "V1": "V1.png",
        "V2": "V2.png",
        "V3": "V3.png",
        "V4": "V4.png",
        ...
        "V6": "V6.png",
        "2V.3V": "2V.3V.png",
        // Adicione aqui outros mapeamentos conforme necessário para Cube, Octahedron, etc.
    };


    // --- 1. CRIAR O HTML DA CALCULADORA ---
    // --- ALTERADO --- Adicionado um container para a imagem
    const calculatorHTML = `
        <div id="calculator-fixed-box">
            <h2 id="calculator-title">Calculadora de Domos</h2>
            
            <div id="dome-image-container-widget" style="text-align: center; margin-bottom: 15px; min-height: 100px;">
                <img id="dome-image-widget" src="" alt="Diagrama do Domo" style="display: none; max-width: 100%; height: auto; border-radius: 4px;">
            </div>
    ...
    // --- NOVO --- Referência para a nossa imagem
    const domeImage = document.getElementById('dome-image-widget');
    ...
    // --- ALTERADO --- Agora as funções de atualização também chamam a função da imagem
    ...
    // --- NOVO --- Função para atualizar a imagem exibida
    function updateDomeImage() {
        const selectedFreq = freqSelect.value;
        const imageName = DOME_IMAGES[selectedFreq];

        if (imageName) {
            domeImage.src = DOME_IMAGE_PATH + imageName;
            domeImage.style.display = 'block';
        } else {
            domeImage.style.display = 'none';
        }
    }

    // Event Listeners para menus dinâmicos
    solidSelect.addEventListener('change', updateFreqOptions);
    // --- ALTERADO --- O menu de frequência agora também atualiza a imagem
